...
void insert_hash(HashT* hash, char data, char data_2)
{
	printf("insert ");
	scanf_s("%s %s", &data, &data_2);
...
int main()
{
	...
	for (int q = 0; q < 20; q++)
	{
		...
		char in = "";
		char kr = "";
		printf("<Bucket Size = 7, Slot = 2>\n");
		printf("Input a menu(i, s, d, x): ");
		scanf_s("%s", &a);
		iarr[q] = a;

		if (iarr[q] == a) {
			insert_hash(hash, arr[q], arr[0]);
		}
	}
